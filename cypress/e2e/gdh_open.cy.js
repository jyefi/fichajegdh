  ...
  it('passes', () => {
    ...
    td.then(($td) => {
      if ($td.hasClass('atrapermisos') || $td.hasClass('traincompleto') || $td.hasClass('atrafestivo')) {
        ...
        cy.writeFile('cypress/logs/warning.log', `${new Date().toLocaleString('es-ES', {day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false}).replace(/,/g, '')}: Check in: You haven't booked today. I'll execute the check in`, { flag: 'a+' });
        cy.wrap(true).should('be.true');
